    ...
    public class CommandService : ICommandService
    {
        ...
        public async Task<IResult> ExecuteAsync(Command command, string rawArguments, CommandContext context)
        {
            ...
            try
            {
                ...
                if (command.CustomArgumentParserType != null)
                {
                    ...
                    {
                        throw new InvalidOperationException(
                            $"Custom argument parser of type {command.CustomArgumentParserType} for command {command} not found.");
                    }
                }
                else
                {
            ...
            {
                var executionFailedResult =
                    new ExecutionFailedResult(command, CommandExecutionStep.ArgumentParsing, ex);
                await InvokeCommandErroredAsync(executionFailedResult, context).ConfigureAwait(false);
                return executionFailedResult;
            }
        ...
        internal IPrimitiveTypeParser GetPrimitiveTypeParser(Type type)
        {
            ...
            if (type.IsEnum)
            {
                ...
                _primitiveTypeParsers.TryAdd(Utilities.MakeNullable(type),
                    Utilities.CreateNullableEnumTypeParser(type.GetEnumUnderlyingType(), enumParser));
                return enumParser;
            }
        ...
        private async Task<IResult> InternalExecuteAsync(CommandContext context)
        {
            ...
            {
                var result = new ExecutionFailedResult(context.Command,
                    CommandExecutionStep.CooldownBucketKeyGenerating, ex);
                await InvokeCommandErroredAsync(result, context).ConfigureAwait(false);
                return result;
            }
        ...
        private async Task<(FailedResult FailedResult, object[] ParsedArguments)> CreateArgumentsAsync(
            ArgumentParserResult parserResult, CommandContext context)
        {
            if (context == null)
            {
                throw new ArgumentNullException(nameof(context), "The context must not be null.");
            ...
            for (var i = 0; i < context.Command.Parameters.Count; i++)
            {
                ...
                {
                    throw new InvalidOperationException(
                        $"No value for the required parameter {parameter.Name} ({parameter.Type}) was returned by the argument parser ({context.Command.CustomArgumentParserType ?? DefaultArgumentParser.GetType()}).");
                }
                ...
                if (!parameter.IsMultiple)
                {
                    var (result, parsedArgument) =
                        await ParseArgumentAsync(parameter, value, context).ConfigureAwait(false);
                    if (result != null)
                    {
                ...
                else
                {
                    ...
                    {
                        throw new InvalidOperationException(
                            "The multiple parameter requires an enumerable of objects as its argument.");
                    }
                    ...
                    if (value is IReadOnlyList<object> argumentsList)
                    {
                        ...
                        {
                            var (result, parsedArgument) =
                                await ParseArgumentAsync(parameter, argumentsList[j], context).ConfigureAwait(false);
                            if (result != null)
                            {
                                return (result, default);
                            }
                    ...
                    else
                    {
                        ...
                        {
                            var (result, parsedArgument) = await ParseArgumentAsync(parameter, argument, context)
        ...
        private async Task<(TypeParseFailedResult TypeParseFailedResult, object ParsedArgument)> ParseArgumentAsync(
            Parameter parameter, object argument, CommandContext context)
        {
            ...
            if (parameter.CustomTypeParserType != null)
            {
                ...
                {
                    throw new InvalidOperationException(
                        $"Custom type parser of type {parameter.CustomTypeParserType} for parameter {parameter} not found.");
                }
            ...
            var parser = GetAnyTypeParser(parameter.Type,
                (primitiveParser = GetPrimitiveTypeParser(parameter.Type)) != null);
            if (parser != null)
            {
            ...
            {
                throw new InvalidOperationException(
                    $"No type parser found for parameter {parameter} ({parameter.Type}).");
            }
        ...
        {
            return _commandExecuted.InvokeAsync(new CommandExecutedEventArgs(result, context));
        }

        private Task InvokeCommandErroredAsync(ExecutionFailedResult result, CommandContext context)
        {
            return _commandExecutionFailed.InvokeAsync(new CommandExecutionFailedEventArgs(result, context));
        }
    }
}