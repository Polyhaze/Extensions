...
namespace Ultz.Extensions.Commands.Built
{
    ...
    public sealed class Parameter
    {
            ...
            for (var i = 0; i < builder.Checks.Count; i++)
            {
                ...
                if (check.Predicate != null && !check.Predicate(Type))
                {
                    throw new ParameterBuildingException(builder,
                        $"{check} is not a valid parameter check for a parameter of type {Type}.");
                }
        ...
        /// <summary>
        /// Gets the <see cref="Built.Command" /> of this <see cref="Parameter" />.
        /// </summary>
        ...
        public async Task<IResult> RunChecksAsync(object argument, CommandContext context)
        {
            if (Checks.Count > 0)
            {
                async Task<(ParameterCheckAttribute Check, CheckResult Result)> RunCheckAsync(
                    ParameterCheckAttribute check)
                {
                ...
                {
                    return new ParameterChecksFailedResult(this, argument,
